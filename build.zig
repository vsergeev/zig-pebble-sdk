...
fn pebble_header_fixup(b: *std.Build, pebble_include_path: []const u8) []const u8 {
    var pebble_header = std.Io.Dir.cwd().readFileAlloc(
        ...
        error.FileNotFound => {
            std.process.fatal("'pebble.h' not found in '{s}'. Set 'pebble_sdk_path' build option to the correct path.", .{pebble_include_path});
            std.process.cleanExit();
        },
        else => std.debug.panic("Error while looking for 'pebble.h' in '{s}' : {t}", .{ pebble_include_path, err }),
    };
